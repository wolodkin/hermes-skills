...
# SiYuan inline tags: #name# (no parentheses; () are invalid in tag names)
DEFAULT_TAG = "#HermInE#"
...
def append_entry(
    ...
    result = siyuan_post(
        ...
        {"parentID": doc_id, "data": content, "dataType": "markdown"},
    )
    if result.get("code") != 0:
...
def main() -> int:
    ...
    tag = state.get("daily_tag") or DEFAULT_TAG
    ...
    try:
        append_entry(base_url, token, doc_id, user_text, tag)
    except RuntimeError:
        doc_id = find_day_doc_by_attribute(base_url, token, ymd)
        if not doc_id:
            doc_id = find_day_doc_id(base_url, token, notebook_id, hpath)
