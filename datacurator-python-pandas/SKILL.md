...
description: Activate with "Hi datacurator-python-pandas" to run as collection curator with enriched system prompt; exit with "Ciao". Discover, profile, sample, and query Senckenberg CSVs read-only via bundled scripts.
version: 1.0.0
...
# datacurator-python-pandas Skill
...
datacurator-python-pandas runs in a **delegated mode**: a dedicated model context with the full curator system prompt (rich collection context). Normal Hermes resumes after exit.
...
- Contains the word `datacurator-python-pandas` (substring match is enough)

Examples: `Hi datacurator-python-pandas`, `hi datacurator-python-pandas`, `Hi  datacurator-python-pandas`
...
1. Run discovery and build the enriched system prompt:
   ...
   `python3 ~/.hermes/skills/datacurator-python-pandas/scripts/build_prompt.py`
...
3. Set Hermes memory: `datacurator-python-pandas.session_active` = `true` (and optionally store `datacurator-python-pandas.activated_at` ISO timestamp).
...
While `datacurator-python-pandas.session_active` is true, **all user messages** in this session are handled in **curator mode** (same system prompt, use bundled scripts for data questions). Do not revert to the default Hermes persona mid-session unless the user deactivates.
...
1. Set `datacurator-python-pandas.session_active` = `false`.
...
3. Confirm briefly that datacurator-python-pandas mode ended and Hermes continues as usual.
4. Do **not** run collection scripts unless the user starts a new session with `Hi datacurator-python-pandas` again.

### If already active

- `Hi datacurator-python-pandas` again: optional refresh (`build_prompt.py --refresh`); stay in curator mode.
...
```bash
python3 ~/.hermes/skills/datacurator-python-pandas/scripts/<script>.py ...
```
...
State file: `~/.hermes/state/datacurator-python-pandas.json`
